...
在 Agentic AI 的背景下，工具是 LLM 可以调用的外部功能，例如：
- API
- 数据库查询
- 内部服务
- 第三方系统
...
它们将 LLM 从一个只会**说话**的东西转变为一个能够**行动**的东西。
...
1. 用户提出问题或给出任务。
2. LLM 理解需要做什么，并规划下一步行动。
3. 解析器将 LLM 的想法转换为结构化格式（如 `get_order_status(order_id=12345)`）。
...
**重要提醒：**
LLM 本身仍然只是生成文本。
这些文本被构造成工具调用，在外部执行，结果被反馈回 LLM — 形成一个推理、行动和反思的循环（**也就是智能体**）。
